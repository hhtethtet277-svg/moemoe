...
async def auto_loop_bypass(session_url, mac_address, voucher, gateway_ip):
    ...
    print(f"  ⏱️  Interval: {loop_interval // 60} minutes")
    ...
    while auto_loop_running:
        ...
        if success:
            ...
            print("\n  ⚠️ Bypass failed, retrying in 10 minutes...")
        
        # Wait for 10 minutes
        print(f"\n  ⏳ Waiting {loop_interval // 60} minutes before next loop...")
        for i in range(loop_interval, 0, -10):
            if not auto_loop_running:
                break
            if i % 60 == 0:
                print(f"  ⏱️  {i // 60} minutes remaining...")
...
def start_bypass():
    ...
    print("\033[1;33m[+] Enter WiFi Details (Press Enter to use saved value)\033[0m\n")
    ...
    mac_address = input("\033[1;32m=> MAC Address: \033[0m").strip() or old_mac
    ...
    if success:
        ...
        # Ask for auto loop
        print("\n" + "="*56)
        print("\033[1;33m[?] Start auto bypass loop (every 10 minutes)?\033[0m")
        auto_choice = input("\033[1;32m=> (y/n): \033[0m").strip().lower()
        ...
        print("\n\033[1;31m[-] Initial bypass failed. Please check your details.\033[0m")
